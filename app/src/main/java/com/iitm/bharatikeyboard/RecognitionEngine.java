...
public class RecognitionEngine {
	...
	public RecognitionEngine(List<Stroke> strokeList){
		...
		//		String sss="";
		//		for(Stroke ss:strokeList){
		//			sss += StraightLineChecker.isLine(ss);
		//		}
		//		Log.i(LOG,sss);
...
//			System.out.println("Main Stroke identification Stroke "+i+" Conditional boolean values "+
//					((( Math.abs(Math.min(currStrokeYMax,mainYMax) - Math.max(currStrokeYMin,mainYMin)) )
...
//							<MAIN_X_OVERLAP_THRESHOLD * Math.min(currStrokeWidth,mainWidth)))))+ " "+
//							
//					((xspace>CHAR_TO_CHAR_GAP_MAIN)	&& ((currStrokeHeight/mainHeight)>(PERCENT_SIZE_FOR_MAIN/2))) //for halant again
//							
//					);
			
//			System.out.println("Dev "+((((currStrokeHeight/mainHeight)>PERCENT_SIZE_FOR_MAIN))) +" "+(( Math.abs(Math.min(currStrokeXMax,mainXMax) - Math.max(currStrokeXMin,mainXMin)) ))
//					+" "+(MAIN_X_OVERLAP_THRESHOLD * Math.min(currStrokeWidth,mainWidth)) +  " "+(currStrokeXMin>mainXMax));
//			
			...
			if((  ((( (Math.min(currStrokeYMax,mainYMax) - Math.max(currStrokeYMin,mainYMin)) )
				...
				if((xspace<0) && isStrokeY(strokeList.get(mainStrokeIndex),strokeList.get(i))){
					//Log.i("Dev","xspace while ignoring y "+xspace);
					continue;
				}
	...
	private void splitWordsEnglish(){
		...
		for(int i=(mainStrokeIndex+1);i<strokeList.size();i++){
			...
			if( (currStrokeYMin<mainYMax) && (currStrokeYMax>mainYMin) && (i-1>=0) 
				  ...
				  //&& ((strokeList.get(i).xMin - strokeList.get(i-1).xMax)>CHAR_TO_CHAR_GAP_ENGLISH)//&& ( ((!isIntersecting(strokeList.get(i-1),strokeList.get(i))) 
...
//		if( (oneStroke.xMin > INTERSECTION_THRESHOLD+twoStroke.xMax) || (twoStroke.xMin >INTERSECTION_THRESHOLD+ oneStroke.xMax) 
//				|| (twoStroke.yMin > INTERSECTION_THRESHOLD+ oneStroke.yMax) || (oneStroke.yMin > INTERSECTION_THRESHOLD+twoStroke.yMax) ){
//			Log.e("Dev","isIntersecting return is false");
//			return false; 
//		}
	...
	public String recognizeBharatiWord(SVParams mainSVParams,SVParams topSVParams,LanguageRules languageRules){
		...
		boolean finishedMainStroke = false;
			...
			for(int i=0;i<sg.size();i++){ //for every stroke in character
				...
				//else if((currentStroke.size()<3)||(( (currentStroke.height < (0.1*(mainStroke.height)))
				//		&& ((currentStroke.width)<(0.1*(mainStroke.width))) )) ){ //0.1 * main stroke
				...
				if(!finishedMainStroke){
					...
					//currentStroke = Preprocessor.interpolate(currentStroke,60);

					//Straight Line case
					//@deprecated
					// if((currentStroke.size()>2) && (StraightLineChecker.isLine(currentStroke))){
					/*if((currentStroke.size()>2) && (!StraightLineChecker.isLine(currentStroke)) &&
							mainStroke.xMin>currentStroke.xMin && currentStroke.xMax<mainStroke.xMax
							&& currentStroke.yMin > mainStroke.yMax){
						if(sg.size() > 1){
							y_base_flag = 1;
				...
				//Auxiliary Stroke Recognition
					...
					//					Log.i(LOG,"currentStroke.yMin "+currentStroke.yMin
					//							+" mainStroke.yMin "+mainStroke.yMin+" isline "+StraightLineChecker.isLine(currentStroke));
			...
			} //end of looping over all strokes in character
			//Stroke mainStroke = sg.get(0);
			//wordLabel += applyBharatiRules(languageRules, output_char_consonant, output_char_vowel) + " ";

			/*if(sg.size()>=2 && (based_char_flag==1||base_char_flag==2)) {
				Stroke firstStroke = sg.get(0);
				Stroke secondStroke = sg.get(1);
